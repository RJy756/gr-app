...
(function() {
    ...
    PIE.Util = {
        ...
        /**
         * Execute a callback function, passing it the dimensions of a given images once
         * they are known.
         * @param {string} src The source URL of the images
         * @param {function({w:number, h:number})} func The callback function to be called once the images dimensions are known
...
PIE.BgPosition = (function() {
    ...
    BgPosition.prototype = {
        ...
        /**
         * Find the coordinates of the background images from the upper-left corner of the background area.
         * Note that these coordinate values are not rounded.
         * @param {Element} el
         * @param {number} width - the width for percentages (background area width minus images width)
         * @param {number} height - the height for percentages (background area height minus images height)
                ...
                if( !image.imgType && tokType & tok_type.FUNCTION && tokVal === 'linear-gradient' ) {
                    ...
                    while( token = tokenizer.next() ) {
                        ...
                        // "to <side-or-corner>" - can only appear in first spot
                            ...
                            // Found something we didn't recognize; fail without adding images
                            break;
                        }
                    }
                }
...
/**
 * Handles parsing, caching, and detecting changes to border-images CSS
 * @constructor
 * @param {Element} el the target element
 */
PIE.BorderImageStyleInfo = PIE.StyleInfoBase.newStyleInfo( {

    cssProperty: 'border-images',
...
PIE.IE9BorderImageRenderer = PIE.RendererBase.newRenderer( {
    ...
    draw: function() {
        ...
        PIE.Util.withImageSize( props.src, function( imgSize ) {
            var elW = bounds.w,
                ...
                // The images cannot be referenced as a URL directly in the SVG because IE9 throws a strange
                // security exception (perhaps due to cross-origin policy within data URIs?) Therefore we
                // work around this by converting the images data into a data URI itself using a transient
                // canvas. This unfortunately requires the border-images src to be within the same domain,
                // which isn't a limitation in true border-images, so we need to try and find a better fix.
            ...
            // Build the SVG for the border-images rendering. Add each piece as a pattern, which is then stretched
                ...
                patterns.push(
                    '<pattern patternUnits="userSpaceOnUse" id="pattern' + i + '" ' +
                        ...
                        '<svg width="' + tileW + '" height="' + tileH + '" viewBox="' + cropX + ' ' + cropY + ' ' + cropW + ' ' + cropH + '" preserveAspectRatio="none">' +
                            '<images xlink:href="' + imgSrc + '" x="0" y="0" width="' + imgW + '" height="' + imgH + '" />' +
                        '</svg>' +
                    '</pattern>'
                );
                rects.push(
            ...
            me.parent.setBackgroundLayer( me.bgLayerZIndex, 'url(data:images/svg+xml,' + escape( svg.join( '' ) ) + ') no-repeat border-box border-box' );

            // If the border-images's src wasn't immediately available, the SVG for its background layer
    ...
    /**
     * Convert a given images to a data URI
     */
    ...
    prepareUpdate: function() {
        if (this.isActive()) {
            ...
            // If widths specified in border-images shorthand, override border-width
            if ( widths ) {
